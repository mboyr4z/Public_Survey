...
namespace Survey.Infrastructure.Extensions
{
    public static class FilteringExtensions
    {
        ...
        public static RegisteredCommentators FilteringRegisteredCommentatorsByIdentityUser(
         this RegisteredCommentators registeredCommentators, SurveyUserRequestParameters surveyUserParameters, IdentityUserRequestParameters identityUserParameters)
        {

            if (!string.IsNullOrEmpty(identityUserParameters.Email))
            {
                registeredCommentators.commentatorList = registeredCommentators.commentatorList.Where(commentatorItem => commentatorItem.user.Email.ToLower().Contains(identityUserParameters.Email.ToLower())).ToList();
            }

            if (!string.IsNullOrEmpty(identityUserParameters.Username))
            {
                registeredCommentators.commentatorList = registeredCommentators.commentatorList.Where(commentatorItem => commentatorItem.user.UserName.ToLower().Contains(identityUserParameters.Username.ToLower())).ToList();
            }

            if (!string.IsNullOrEmpty(surveyUserParameters.Name))
            {
                registeredCommentators.commentatorList = registeredCommentators.commentatorList.Where(commentatorItem => commentatorItem.commentator.Name.ToLower().Contains(surveyUserParameters.Name.ToLower())).ToList();
            }

            if (!string.IsNullOrEmpty(surveyUserParameters.Surname))
            {
                registeredCommentators.commentatorList = registeredCommentators.commentatorList.Where(commentatorItem => commentatorItem.commentator.Surname.ToLower().Contains(surveyUserParameters.Surname.ToLower())).ToList();
            }

            if (!string.IsNullOrEmpty(surveyUserParameters.minLikeRate.ToString()))
            {
                registeredCommentators.commentatorList = registeredCommentators.commentatorList.Where(commentatorItem => commentatorItem.commentator.LikeRate > surveyUserParameters.minLikeRate).ToList();
            }

            if (!string.IsNullOrEmpty(surveyUserParameters.maxLikeRate.ToString()))
            {
                registeredCommentators.commentatorList = registeredCommentators.commentatorList.Where(commentatorItem => commentatorItem.commentator.LikeRate < surveyUserParameters.maxLikeRate).ToList();
            }

            return registeredCommentators;
        }
